...
define("E_VAL_PHONE_CHECK_FAILED","Invalid phone number");
define("E_VAL_FAX_CHECK_FAILED","Invalid fax number");
define("E_VAL_COUNTRY_CHECK_FAILED","Please select any country");
define("E_VAL_ZIP_CHECK_FAILED","Please enter zip code");
define("E_VAL_NAME_CHECK_FAILED","Please enter only alpha values");
...
class FormValidator 
{
  ...
  function GetErrors()
  {
	...
	{
	  $bret = true;
  	if(!isset($input_value) || strlen($input_value) <=0 || trim($input_value) == '')
		{
			$bret=false;
	...
	{
    return preg_match('/^[a-zA-Z][\w\.-]*[a-zA-Z0-9]@[a-zA-Z0-9][\w\.-]*[a-zA-Z0-9]\.[a-zA-Z][a-zA-Z\.]*[a-zA-Z]$/', $email);
	}

	function validate_phone($phone) 
	{
		return preg_match('/^[0-9\-\+\s]{4,}$/', $phone);
	}

	function validate_fax($fax) 
	{
		return preg_match('/^[0-9\-\+\s]{6,}$/', $fax);
	}
	...
	{
		return preg_match('/^[a-zA-Z0-9-\+\s]{5,}$/', $zip);
	}
			...
			case 'phone':
						{
							if(isset($input_value) && strlen($input_value)>0)
							{
								$bret= $this->validate_phone($input_value);
								if(false == $bret)
								{
									$default_error_message = E_VAL_PHONE_CHECK_FAILED;
								}
							}
							break;
						}
			case 'fax':
						{
							if(isset($input_value) && strlen($input_value)>0)
							{
								$bret= $this->validate_fax($input_value);
								if(false == $bret)
								{
									$default_error_message = E_VAL_FAX_CHECK_FAILED;
								}
							}
							break;
						}
			case 'country':
						{
							if(isset($input_value) && strlen($input_value)>0)
							{
								$bret= $this->validate_country($input_value);
								if(false == $bret)
								{
									$default_error_message = E_VAL_COUNTRY_CHECK_FAILED;
								}
							}
							break;
						}
			case 'zip':
						{
							if(isset($input_value) && strlen($input_value)>0)
							{
								$bret= $this->validate_zip($input_value);
								if(false == $bret)
								{
									$default_error_message = E_VAL_ZIP_CHECK_FAILED;
								}
							}
							break;
						}
